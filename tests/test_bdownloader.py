...
class TestBDownloader(unittest.TestCase):
    def setUp(self):
        self.tmp_dir = tempfile.mkdtemp(prefix='bdl-tmp-')
    ...
    def test_bdownloader_download(self):
        file_path = os.path.join(self.tmp_dir, "aria2-x86_64-win.zip")
    ...
    def test_bdownloader_downloads(self):
        files = [
            {
                "file": os.path.join(self.tmp_dir, "aria2-x86_64-linux.tar.xz"),
            ...
            {
                "file": os.path.join(self.tmp_dir, "mkvtoolnix-x86_64-linux.tar.xz"),
